              ...
              <tr key={item.id} className="transition-colors duration-150 last:border-0"
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={cn("text-xs", item.ativo ? "text-[#6ee7b7]" : "text-muted-foreground")}>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="transition-colors duration-150 last:border-0"
                ...
                <td className="px-4 py-3">
                  <Badge className={cn("text-xs border", item.ativo ? "bg-[#6ee7b7]/15 text-[#6ee7b7] border-[#6ee7b7]/20" : "bg-muted text-muted-foreground border-border")}>
                    {item.ativo ? "Ativo" : "Inativo"}
                  </Badge>
                </td>
                <td className="px-4 py-3">
        ...
        >
          {(["all", ...atributoCategories] as const).map((cat) => (
            <button
              ...
              style={activeCategory === cat ? { background: "#10b981", color: "#fff", boxShadow: "0 0 10px rgba(57,217,138,0.20)" } : undefined}
            >
              {cat === "all" ? "Todos" : cat}
            </button>
          ))}
              ...
              <tr key={item.id} className="transition-colors duration-150 last:border-0"
                ...
                <td className="px-4 py-3">
                  <span className="px-2 py-0.5 rounded-full text-xs font-medium"
                    style={{ background: "rgba(255, 255, 255, 0.09)", border: "1px solid rgba(16, 185, 129, 0.22)", color: "#6ee7b7" }}>
                    {item.categoria}
                  </span>
                </td>
                <td className="px-4 py-3">
